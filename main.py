...
def criar_log_Json():
    ...
    # gerando maquina e sensores
    maquinas = []
    for _ in range(random.randint(2, 4)):
        # nome do maquina aleatório
        nome_maquina = ''.join(random.choice(letras) for i in range(7))
        # sensores do maquina
        sensores = []
        ...
        # adiciondo maquina
        maquinas.append({'maquina': nome_maquina, 'sensores': sensores})
    ...
    for instante in instantes:
        for maquina in maquinas:
            ...
            for s, v in zip(maquina['sensores'], valores):
                medida[s['sensor']] = v
...
if __name__ == '__main__':
    # Iniciando carregando os arquivos em formato texto exportados do Visual Object Net ++ (VON)
    # Menu de opções prontas:
    arq = opcoes()
    ...
    # gviz = pm.visualization.petri_net.visualizer.apply(rdp_teste, rdp_marcacao_inicial, rdp_marcacao_final)
    # image = pm.visualization.petri_net.visualizer.view(gviz)
    criar_log_Json()